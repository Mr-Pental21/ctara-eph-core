...
Elixir bhava config maps accept `:use_rashi_bhava_for_bala_avastha` and `:include_rashi_bhava_results`; both default to `true`. They also accept `:include_node_aspects_for_drik_bala`, defaulting to `false`, to include Rahu/Ketu incoming aspects in Shadbala Drik Bala. Existing bhava fields keep configured-system meaning. Rashi-bhava sibling keys such as `:rashi_bhava_cusps`, `:rashi_bhava_number`, and `:graha_to_rashi_bhava` expose the equal-house/whole-sign companion basis.
