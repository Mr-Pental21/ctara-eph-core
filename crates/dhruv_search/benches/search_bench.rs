...
fn shadbala_for_date_bench(c: &mut Criterion) {
    let engine = match load_engine() {
        Some(v) => v,
        None => return,
    };
    ...
    group.sample_size(10);
    group.bench_function("shadbala_for_date", |b| {
        b.iter(|| {
            dhruv_search::shadbala_for_date(
                black_box(&engine),
                black_box(&eop),
                black_box(&utc),
                black_box(&location),
...
fn vimsopaka_for_date_bench(c: &mut Criterion) {
    let engine = match load_engine() {
        Some(v) => v,
        None => return,
    };
    ...
    group.sample_size(10);
    group.bench_function("vimsopaka_for_date", |b| {
        b.iter(|| {
            dhruv_search::vimsopaka_for_date(
                black_box(&engine),
                black_box(&eop),
                black_box(&utc),
                black_box(&location),
...
fn avastha_for_date_bench(c: &mut Criterion) {
    let engine = match load_engine() {
        Some(v) => v,
        None => return,
    };
    ...
    let utc = UtcTime::new(2024, 1, 15, 6, 30, 0.0);
    let location = GeoLocation::new(28.6139, 77.2090, 0.0);
    let bhava_config = BhavaConfig::default();
    let rs_config = RiseSetConfig::default();
    let aya_config = SankrantiConfig::default_lahiri();
    ...
    group.sample_size(10);
    group.bench_function("avastha_for_date", |b| {
        b.iter(|| {
            dhruv_search::avastha_for_date(
                black_box(&engine),
                black_box(&eop),
                black_box(&location),
                black_box(&utc),
