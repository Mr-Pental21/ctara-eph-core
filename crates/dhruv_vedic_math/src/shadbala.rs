...
mod tests {
    ...
    fn drik_bala_excludes_node_aspects_by_default_and_can_include_them() {
        let mut lons = [0.0f64; 9];
        lons[Graha::Surya.index() as usize] = 0.0;
        lons[Graha::Rahu.index() as usize] = 180.0;
        lons[Graha::Ketu.index() as usize] = 0.0;
    ...
    fn saptavargaja_moolatrikona_is_degree_specific() {
        ...
        assert!(
            (saptavargaja_bala(Graha::Buddh, &varga_rashi, &varga_longitudes) - 225.0).abs() < EPS
        );
        assert!(
        ...
        assert!(
            (saptavargaja_bala(Graha::Buddh, &varga_rashi, &varga_longitudes) - 183.75).abs() < EPS
        );
        assert!(
