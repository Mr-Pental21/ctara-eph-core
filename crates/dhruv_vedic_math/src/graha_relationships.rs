...
///
/// `d1_rashi_indices` = 7 sapta graha D1 rashi positions (indexed 0-6).
...
pub fn buddh_association_nature_with_chandra_rule(
    ...
    let Some(buddh_rashi) = longitude_rashi_index(sidereal_lons[Graha::Buddh.index() as usize])
    else {
...
mod tests {
    ...
    fn moolatrikone_moon_and_mercury_ranges_start_after_exaltation_zone() {
        assert_eq!(moolatrikone_range(Graha::Chandra), Some((1, 3.0, 30.0)));
    ...
    fn dignity_moon_taurus_exaltation_then_moolatrikona() {
        assert_eq!(
            dignity_in_rashi(Graha::Chandra, 32.999, 1),
            Dignity::Exalted
        );
        assert_eq!(
